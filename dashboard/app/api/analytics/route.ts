...
export async function GET(req: NextRequest) {
  ...
  const [{ count: scheduledThisWeek }, { count: trendsToday }] = await Promise.all([
    db.from('content_queue').select('id', { count: 'exact', head: true })
      .eq('workspace_id', workspaceId).gte('created_at', weekAgo),
    db.from('trends').select('id', { count: 'exact', head: true })
      .eq('workspace_id', workspaceId).gte('created_at', `${today}T00:00:00Z`),
  ])

  const latest = snapshots?.[0]
  const totalViews7d = latest
    ? Object.values(latest.platform_metrics ?? {}).reduce(
        (sum: number, m: unknown) => sum + ((m as { views: number }).views ?? 0), 0)
    : 0
  ...
  const avgEngagement = engagements.length > 0
    ? engagements.reduce((sum, m) => sum + (m.views > 0 ? (m.likes / m.views) * 100 : 0), 0) / engagements.length
    : 0
