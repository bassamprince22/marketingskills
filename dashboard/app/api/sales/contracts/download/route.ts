...
export async function POST(req: NextRequest) {
  ...
  const { fields, filename } = await req.json() as { fields: Record<string, string>; filename?: string }

  const db = getServiceClient()
  const { data, error } = await db.storage.from(BUCKET).download(TEMPLATE)
  if (error || !data) return NextResponse.json({ error: 'No template found' }, { status: 404 })
